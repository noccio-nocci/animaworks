...
def person_dir(tmp_path, monkeypatch):
    """Create an isolated person directory with sample knowledge files.
    ...
    (person / "knowledge" / "chatwork-policy.md").write_text(
        "# Chatwork対応方針\n\n"
    ...
    (person / "knowledge" / "slack-integration.md").write_text(
        "# Slack連携\n\n"
        "## 通知設定\n\n重要なアラートはSlackに通知する。\n\n"
        "## チャンネル\n\n- #general: 一般連絡\n- #alerts: 緊急アラート\n",
        encoding="utf-8",
    ...
    (person / "knowledge" / "important-rule.md").write_text(
        "# 重要ルール\n\n"
...
def test_access_frequency_full_pipeline(person_dir, vector_store, indexer, retriever):
    ...
    retriever.record_access(results, "test_person")
    ...
    retriever.record_access(results2, "test_person")
...
def test_synaptic_downscaling_e2e(person_dir, vector_store, indexer):
    ...
    engine = ForgettingEngine(person_dir, "test_person")
...
def test_complete_forgetting_e2e(person_dir, vector_store, indexer):
