...
> 作成日: 2026-02-19
> 関連: [vision.md](vision.md), [memory.md](memory.md)
...
この区別はvision.mdの「不完全な個」の設計思想と整合する。事前学習知識だけでは足りないからこそ、経験に基づく記憶システムが必要になる。
...
人間の組織が機能するのは、各メンバーが限られた視野と記憶で判断し、不完全な情報を自分の言葉で伝え合うからである（vision.md）。これは認知負荷理論（Sweller, 1988）と分散認知（Hutchins, 1995）の知見と整合する。
