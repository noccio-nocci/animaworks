...
| `send_message` | 1対1の依頼・報告・質問 | `intent` 必須: `report` / `question` のいずれか。タスク委譲（直属部下）は `delegate_task` を使用（`delegation` は廃止済み）。人間エイリアス宛ては外部チャネル（Slack/Chatwork等）へ配信 |
...
直属部下でない相手への依頼は `send_message` で `intent="question"` を指定する（`delegation` は廃止済み）。
...
ハートビートや cron に組み込むべき定期タスクを指示するパターン。継続タスクは `delegate_task` の対象外のため、`send_message` で `intent="question"` を指定する（`delegation` は廃止済み）。
