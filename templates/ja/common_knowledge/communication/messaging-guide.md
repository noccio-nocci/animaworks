...
**廃止**: `intent="delegation"` は廃止されました。タスク委譲には `delegate_task` ツールを使用してください。
...
| `intent` | 送信者の意図 | `report`, `question`（委譲は delegate_task 経由で `delegation` が付与される） |
...
**原因**: `intent` を省略した、`intent="delegation"` を使った（廃止済み）、または acknowledgment・感謝・FYI を DM で送ろうとした
